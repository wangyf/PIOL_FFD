...
case( '=r', '+r', '=R', '+R' )
    ...
    if ( io%ib == io%nb ) then
        ...
        if ( (any( io%buff(:,:n(4)) /= io%buff(:,:n(4)) ) .or. &
            maxval( io%buff(:,:n(4)) ) > huge( val ) ).and. .not. bigendian) then
...
if (verb) write(0,*) 'iopointer->',io%field,'mode=',io%mode,'pass=',pass
! 4D slice
i1 = io%ii(1,1:3) - nnoff
i2 = io%ii(2,1:3) - nnoff
di = io%ii(3,1:3)
...
!write assembled field non-collective at master processor
subroutine mstrwrite2d(f,filename)
real,intent(in) :: f(:,:)
character(*) :: filename

integer :: i,j

inquire( iolength=i ) f(:,:)
open(1, file=filename, recl=i, form='unformatted', access='direct', &
        status='replace' )
        
write( 1, rec=1 ) ((f(i,j),i=1,size(f,1)),j=1,size(f,2))
close(1)
end subroutine

!write assembled field non-collective at master processor
subroutine mstrwrite1d(f,filename)
real,intent(in) :: f(:)
character(*) :: filename
...
write( 1, rec=1 ) (f(i),i=1,size(f,1))
close(1)
end subroutine
end module
